...
map('n', '<leader>f', ':lua require"telescope.builtin".grep_string(require("telescope.themes").get_ivy({}))<cr>', default_opts)
map('n', '<leader>l', ':lua require"telescope.builtin".live_grep(require("telescope.themes").get_ivy({}))<cr>', default_opts)
...
map("n", "<leader><F2>", "<cmd>lua require 'telescope'.extensions.file_browser.file_browser({ cwd = vim.fn.expand('%:p:h') })<CR>", default_opts)
